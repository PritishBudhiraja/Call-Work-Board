...
const mapStateToProps = (state) => {
  return {
    dataUserCallBookThird: state.callWorkBoard.dataUserCallbook,
  };
};
...
export default connect(
  ...
  const { dataUserCallBookThird = {} } = props;
  const classes = useStyles();
  const { workbookItems = [] } = dataUserCallBookThird;
  const data = workbookItems[2];
  const highChartRender = () => {
    if (data) {
      ...
      const chart = Highcharts.chart("thirdCard", {
        chart: {
          type: "column",
      ...
      {data ? (
        <div>
          ...
          <div className="chart-image" style={{ backgroundColor: "#273D49BF" }}>
            <div id="thirdCard" style={{ width: "70%" }}></div>
            <div style={{ width: "30%" }}>
