...
const mapStateToProps = (state) => {
  return {
    dataUserCallBookSecond: state.callWorkBoard.dataUserCallbook,
  };
};
...
export default connect(
  ...
  const { dataUserCallBookSecond } = props;
  const classes = useStyles();
  const { workbookItems = [] } = dataUserCallBookSecond;
  const data = workbookItems[1];
  const highChartRender = () => {
    if (data) {
      ...
      const chart = Highcharts.chart("secondCard", {
        chart: {
          type: "column",
      ...
      {data ? (
        <div>
          ...
          <div className="chart-image" style={{ backgroundColor: "#273D49BF" }}>
            <div id="secondCard" style={{ width: "70%" }}></div>
            <div style={{ width: "30%" }}>
