...
const mapStateToProps = (state) => {
  return {
    dataUserCallBookFifth: state.callWorkBoard.dataUserCallbook,
  };
};
...
export default connect(
  mapStateToProps,
  null
)(function FifthCard(props) {
  const { dataUserCallBookFifth = {} } = props;
  const classes = useStyles();
  const { workbookItems = [] } = dataUserCallBookFifth;
  const data = workbookItems[4];
  const highChartRender = () => {
    if (data) {
      ...
      const chart = Highcharts.chart("fifthCard", {
        chart: {
          type: "column",
  ...
  }, [dataUserCallBookFifth]);

  return (
    <Card
      className={classes.root}
      style={{ backgroundColor: "#273D49BF 0% 0% no-repeat padding-box" }}
    >
      {data ? (
        <div>
          ...
          <div className="chart-image" style={{ backgroundColor: "#273D49BF" }}>
            <div id="fifthCard" style={{ width: "70%" }}></div>
            <div style={{ width: "30%" }}>
