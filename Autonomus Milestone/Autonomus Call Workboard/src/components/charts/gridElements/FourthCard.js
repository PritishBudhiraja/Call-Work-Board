...
const mapStateToProps = (state) => {
  return {
    dataUserCallBookFourth: state.callWorkBoard.dataUserCallbook,
  };
};
...
export default connect(
  ...
  const { dataUserCallBookFourth = {} } = props;
  const classes = useStyles();
  const { workbookItems = [] } = dataUserCallBookFourth;
  const data = workbookItems[3];
  const highChartRender = () => {
    if (data) {
      ...
      const chart = Highcharts.chart("fourthCard", {
        chart: {
          type: "column",
      ...
      {data ? (
        <div>
          ...
          <div className="chart-image" style={{ backgroundColor: "#273D49BF" }}>
            <div id="fourthCard" style={{ width: "70%" }}></div>
            <div style={{ width: "30%" }}>
